...
const ModernToast = ({ alert, setAlert }) => {
  ...
  useEffect(() => {
    if (alert?.message) {
      ...
      const timer = setTimeout(() => {
        setIsVisible(false);
        setTimeout(() => setAlert(), 300); // Wait for animation to complete
      }, 3000);
  ...
  const getColorClasses = () => {
    if (alert.success) return 'from-green-500 to-emerald-500 border-green-400/30';
  ...
  return (
    <div className={`fixed top-24 right-6 z-50 transform transition-all duration-300 ${
      isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'
    }`}>
      <div className={`glass border rounded-2xl p-4 max-w-sm shadow-2xl ${getColorClasses()}`}>
